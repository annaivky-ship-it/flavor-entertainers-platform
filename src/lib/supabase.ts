...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  throw new Error('Missing Supabase environment variables. Please check your .env file or deployment configuration.')
}
...
export const testConnection = async () => {
  try {
    ...
    const { data, error } = await supabase.from('performers').select('id').limit(1)
